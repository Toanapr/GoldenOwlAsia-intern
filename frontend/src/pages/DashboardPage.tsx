...
export function DashboardPage() {
  return (
    <div className="space-y-10">
      <section className="border-t-4 border-accent bg-white">
        <div className="grid border-x border-b border-slate-200 lg:grid-cols-[1fr_310px]">
          <div className="px-6 py-10 sm:px-9 sm:py-12 lg:px-12 lg:py-16">
            ...
            <div className="mt-8 flex flex-wrap gap-x-6 gap-y-3">
              <Link
                to="/search"
                className="inline-flex min-h-11 items-center gap-3 rounded-lg bg-accent px-6 py-3 text-base font-bold text-white transition-colors duration-200 hover:bg-brand-800 focus-visible:outline-2"
              >
                Tra cứu điểm{" "}
              ...
              <Link
                to="/reports"
                className="inline-flex min-h-11 items-center border-b-2 border-slate-400 px-1 py-2 text-base font-bold text-secondary transition-colors duration-200 hover:border-accent hover:text-accent focus-visible:outline-2"
              >
          ...
          <dl className="grid border-t border-slate-200 bg-muted lg:border-t-0 lg:border-l">
            <div className="flex items-end justify-between border-b-2 border-slate-300 px-6 py-6 lg:block lg:px-8 lg:py-8">
              ...
              <dd className="font-mono text-2xl font-bold tracking-tight text-foreground tabular-nums lg:mt-3 lg:text-3xl">
                1.061.605
              </dd>
            </div>
            <div className="flex items-end justify-between border-b-2 border-slate-300 px-6 py-6 lg:block lg:px-8 lg:py-8">
              ...
              <dd className="font-mono text-2xl font-bold text-foreground tabular-nums lg:mt-3 lg:text-3xl">
                09
              </dd>
            </div>
      ...
      <section aria-labelledby="functions-title">
        <div className="grid gap-3 border-b-2 border-slate-900 pb-4 sm:grid-cols-[1fr_auto] sm:items-end">
          <div>
            <p className="text-xs font-bold text-accent">03 CHỨC NĂNG</p>
            <h2
              id="functions-title"
        ...
        <div className="divide-y divide-slate-200">
          {functions.map(({ icon: Icon, ...item }, index) => (
            <Link
              key={item.to}
              to={item.to}
              className="group grid min-h-11 gap-4 py-6 transition-colors duration-200 hover:bg-white sm:grid-cols-[48px_1fr_auto] sm:items-center sm:px-4 focus-visible:outline-2"
            >
              <span className="font-mono text-sm text-slate-400">
                0{index + 1}
              </span>
              <span className="flex items-start gap-4">
                <span className="mt-0.5 grid size-11 shrink-0 place-items-center rounded-lg border border-slate-300 text-accent transition-colors duration-200 group-hover:border-accent">
                  <Icon
                    className="size-4"
                    strokeWidth={1.8}
                    aria-hidden="true"
              ...
              <ArrowRight
                className="ml-auto hidden size-5 text-slate-400 transition-colors duration-200 group-hover:text-accent sm:block"
                aria-hidden="true"
              />
            </Link>
          ))}
